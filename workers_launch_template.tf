...
resource "aws_autoscaling_group" "workers_launch_template" {
    ...
    content {
      ...
      launch_template {
        launch_template_specification {
          launch_template_id = aws_launch_template.workers_launch_template.*.id[each.key]
          version = lookup(
            each.value,
            ...
            ? aws_launch_template.workers_launch_template.*.latest_version[each.key]
            : aws_launch_template.workers_launch_template.*.default_version[each.key]
          )
        }
  ...
  dynamic "launch_template" {
    ...
    content {
      id = aws_launch_template.workers_launch_template.*.id[each.key]
      version = lookup(
        each.value,
        "launch_template_version",
        lookup(
...
resource "aws_launch_template" "workers_launch_template" {
  ...
  user_data = base64encode(
    local.launch_template_userdata_rendered[each.key],
  )
